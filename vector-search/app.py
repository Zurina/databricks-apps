...
def run_vector_search(prompt: str) -> str:
    ...
    if prompt_vector is None:
        return json.dumps({"error": "Failed to generate embeddings for the prompt."}, indent=2)
    ...
    try:
        ...
        if hasattr(query_result.result, 'data_array') and query_result.result.data_array:
            return json.dumps({"results": query_result.result.data_array}, indent=2)
        return json.dumps({"results": []}, indent=2)
    except Exception as e:
        logging.error(f"Error during vector search: {e}")
        return json.dumps({"error": f"Error during vector search: {e}"}, indent=2)
