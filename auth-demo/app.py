                                ...
                                children=[
                                    dmc.Group(
                                        ...
                                        children=[
                                            ...
                                            dmc.Title(
                                                "Datasummit - Databricks Apps Auth",
                                                order=2,
                                            ),
                                        ],
                                    ),
            ...
            dmc.AppShellMain(
                ...
                children=dmc.Container(
                    [
                            ...
                            [
                                dmc.Paper(
                                    [
                                        ...
                                        dmc.Alert(
                                            id="alert-sp-addrow",
                                            children="Add row status will appear here.",
                                            title="Add Row Status",
                                            color="gray",
                                            withCloseButton=True,
                                            hide=True,
                                            radius="sm",
                        ...
                        dcc.Store(id="obo-action-msg-store"),  # NEW: for add-row messages
...
@callback(
    [
        Output("alert-obo-addrow", "children"),
        Output("alert-obo-addrow", "color"),
        Output("alert-obo-addrow", "hide"),
        Output("alert-obo-addrow", "title"),
        Output("add-row-obo", "loading"),
    ],
    Input("add-row-obo", "n_clicks"),
    State("sql-http-path", "value"),
    running=[(Output("add-row-obo", "loading"), True, False)],
    prevent_initial_call=True,
)
def add_row_obo_callback(n_clicks, http_path):
    if not n_clicks or not http_path:
    ...
    try:
        ...
        if not user_token:
            return (
                [
                    "Error: ",
                    dmc.InlineCodeHighlight(code="X-Forwarded-Access-Token"),
                    " not found in this request's headers. Cannot insert row.",
                ],
                "red",
                False,
                "OBO Token Missing",
        ...
        return (
            f"Failed to add row with OBO authorization: {e}",
            "red",
            False,
            "Error",
            False,
...
@callback(
    [
        Output("alert-sp-addrow", "children"),
        Output("alert-sp-addrow", "color"),
        Output("alert-sp-addrow", "hide"),
        Output("alert-sp-addrow", "title"),
        Output("add-row-sp", "loading"),
    ],
    Input("add-row-sp", "n_clicks"),
    State("sql-http-path", "value"),
    running=[(Output("add-row-sp", "loading"), True, False)],
    prevent_initial_call=True,
)
def add_row_sp_callback(n_clicks, http_path):
    if not n_clicks or not http_path:
        return dash.no_update, dash.no_update, dash.no_update, dash.no_update, False
    ...
    try:
        ...
        return (
            f"Failed to add row with SP authorization: {e}",
            "red",
            False,
            "Error",
            False,
